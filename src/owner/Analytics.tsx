    ...
    <Modal open={open} onClose={onClose} width="max-w-[420px]">
      ...
      <div className="flex justify-end gap-3 px-6 py-5 border-t border-white/[0.04] bg-white/[0.01]">
        ...
        <Button 
          ...
          onClick={handleConfirm}
          loading={loading}
          icon={isGranting ? <Icons.shieldCheck className="w-4 h-4" /> : <Icons.shield className="w-4 h-4" />}
        >
          {loading 
            ? (isGranting ? 'Granting…' : 'Revoking…')
            : (isGranting ? 'Grant Permission' : 'Revoke Permission')
          }
        </Button>
      </div>
...
type FilterMode = 'all' | 'owners' | 'admins';
...
export default function AdminUsersPanel() {
  ...
  const [permRequest, setPermRequest] = useState<PermissionChangeRequest | null>(null);
  ...
  const handlePermissionToggle = useCallback(
    (userId: string, permission: keyof AdminUser['permissions'], newValue: boolean) => {
      setPermRequest({ userId, permission, newValue });
    },
    []
  );
  ...
  const handlePermissionConfirm = useCallback(() => {
    ...
    const { userId, permission, newValue } = permRequest;
    setUsers(prev =>
      prev.map(u =>
        u.id === userId
          ? {
    ...
    const permDef = PERMISSION_DEFS.find(p => p.key === permission);
    setPermRequest(null);
    if (user && permDef) {
      addToast(
        `${newValue ? 'Granted' : 'Revoked'} "${permDef.label}" for ${user.name}`,
        newValue ? 'success' : 'info'
      );
    }
  }, [permRequest, users, addToast]);

  const existingEmails = useMemo(() => users.map(u => u.email), [users]);

  return (
    <div className="relative min-h-screen bg-[#0a0a0f] text-zinc-100">
      <AnimatedBackground />
      ...
      <div className="relative z-10 max-w-3xl mx-auto px-4 sm:px-6 py-10 sm:py-16">
        ...
        >
            ...
            <div>
              <div className="flex items-center gap-3 mb-2">
                <motion.div
                  ...
                  animate={{ scale: 1, rotate: 0 }}
                  transition={{ ...DS.motion.bounce, delay: 0.1 }}
                  className="w-10 h-10 rounded-2xl bg-gradient-to-br from-indigo-500 to-violet-600 flex items-center justify-center shadow-lg shadow-indigo-500/25"
                >
                  <Icons.shieldCheck className="w-5 h-5 text-white" />
                </motion.div>
                <div>
                  <h1 className="text-[24px] font-bold text-zinc-50 tracking-tight">
                    Admin Users
                  </h1>
                  <p className="text-[13px] text-zinc-500 mt-0.5">
                    Manage your team's admin access and permissions
                  </p>
                </div>
              </div>
            </div>
          ...
          <motion.div
            initial={{ opacity: 0, y: 10 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ ...DS.motion.smooth, delay: 0.15 }}
            className="flex items-center gap-5 mt-6"
          >
            <StatBadge label="total" value={users.length} icon={Icons.users} accent />
            <div className="w-px h-5 bg-white/[0.06]" />
            <StatBadge label="owners" value={ownerCount} icon={Icons.crown} />
            <div className="w-px h-5 bg-white/[0.06]" />
            <StatBadge label="admins" value={adminCount} icon={Icons.shield} />
          </motion.div>
        </motion.header>
            ...
            {filteredUsers.length > 0 ? (
              filteredUsers.map((user, index) => (
                <motion.div
                  ...
                  initial={{ opacity: 0, y: 20 }}
                  animate={{ opacity: 1, y: 0 }}
                  transition={{ ...DS.motion.smooth, delay: index * 0.05 }}
                >
                  <AdminUserCard
                    user={user}
              ...
              <EmptyState filtered={search.trim().length > 0 || filter !== 'all'} />
            )}
          </AnimatePresence>
        </div>
      </div>
