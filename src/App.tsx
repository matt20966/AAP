...
const MainPage: React.FC<MainPageProps> = ({
  ...
  const loadFromHistory = (item: QueryHistoryItem) => {
    ...
    if (item.sql) {
      setModal({
        isOpen: true,
        stage: item.result ? "results" : "sql-preview",
        naturalQuery: item.query,
        sql: item.sql,
        fullWebhookResponse: item.webhookResponse || "",
    ...
    >
      ...
      <main className="max-w-5xl mx-auto px-6 py-8 space-y-5 relative z-10">
                      ...
                      <div className="flex items-center gap-2 flex-shrink-0">
                        {item.result && (
                          <span
                            className="w-1.5 h-1.5 rounded-full"
                            style={{
                              background: item.result.success
                                ? "#4ade80"
                                : "#f87171",
                            }}
                          />
        ...
        {!loading && !input && (
          <div className="space-y-4">
            <p
              className="text-xs uppercase tracking-widest font-bold px-1"
              style={{ color: "rgba(100,116,139,0.4)" }}
            >
              Try asking
            </p>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
              {[
                ...
                },
                {
                  ...
                  description: "Storage usage across all databases",
                  icon: (
                    <svg
                      className="w-4 h-4"
                      style={{ color: "#a5b4fc" }}
                ...
                },
                {
                  ...
                  description: "Who's currently connected",
                  icon: (
                    <svg
                      className="w-4 h-4"
                      style={{ color: "#a5b4fc" }}
                ...
                >
                  ...
                  <p className="text-sm font-semibold text-slate-300 group-hover:text-white transition-colors mb-1">
                    {example.label}
                  </p>
                  <p
